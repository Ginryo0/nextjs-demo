...
// EXPORTS
__webpack_require__.d(__webpack_exports__, "default", function() { return /* binding */ MeetUpDetails; });
...
async function getStaticPaths() {
  const client = await external_mongodb_["MongoClient"].connect('mongodb+srv://ginryo:nYLu5PhiBEiEWxOP@cluster0.spv0bgt.mongodb.net/meetups?retryWrites=true&w=majority');
  const db = client.db();
  const meetupCollection = db.collection('meetups'); // obj 1 -> filter criteria = none = all objects
...
async function getStaticProps(context) {
  ...
  const objectId = new external_mongodb_["ObjectId"](meetupId);
